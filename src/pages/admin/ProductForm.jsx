...
export default function ProductForm() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (pr) {
          const p = pr.data;
          setForm({ ...p, category: p.category?._id || p.category, discountedPrice: p.discountedPrice ?? "" });
          setMedia([
        ...
        if (!cancelled) alert(e.response?.data?.message || "Could not load product.");
      } finally {
  ...
  async function save(e) {
    ...
    if (form.discountedPrice !== "" && Number(form.discountedPrice) >= Number(form.sellingPrice)) {
      alert("Discounted price must be lower than selling price.");
      return;
    }
    setSaving(true);
    try {
