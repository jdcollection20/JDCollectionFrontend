...
export default function Login(){
  const{admin,login}=useAuth(); const loc=useLocation();
  const[u,setU]=useState(""),[p,setP]=useState(""),[e,setE]=useState(""),[busy,setBusy]=useState(false),[show,setShow]=useState(false);
  if(admin)return <Navigate to={loc.state?.from||"/admin"} replace/>;
  async function submit(x){x.preventDefault();setE("");setBusy(true);try{await login(u,p)}catch(err){setE(err.response?.data?.message||"Login failed")}finally{setBusy(false)}}
  return <div className="grid min-h-[75vh] place-items-center px-4"><form onSubmit={submit} className="card w-full max-w-md p-8">
    <div className="text-4xl">🧸</div><h1 className="mt-4 text-3xl font-black">JD COLLECTION Admin</h1><p className="mt-1 text-slate-500">Sign in to manage the product catalogue.</p>
